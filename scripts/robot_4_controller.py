...
This file controls the robot's movements to the way points--fed to it from robot_4_server.py.
...
if ( obj[ "init" ] == 1 ):
	...
	bge.logic.robot_4_add_marker = True;
	
	bge.logic.robot_4_robot_body_wPos = copy.deepcopy( obj.worldPosition );
	
	bge.logic.robot_4_rotate = False;
	
	bge.logic.robot_4_time_then = time.time( );
	
	bge.logic.robot_4_laydown_campose_time_then = time.time( );
	
	bge.logic.robot_4_orientation_then = ( obj.worldOrientation.to_euler( )[ 2 ] * 180.0 ) / math.pi;
	
	bge.logic.robot_4_magnitude_then = ( obj.worldPosition - bge.logic.robot_4_robot_body_wPos ).magnitude;
	
	bge.logic.robot_4_inc_then = 0;
	
	bge.logic.robot_4_wait_to_go_forward = 0;
	
	bge.logic.robot_4_angle_moved_sum = 0;
	
	bge.logic.robot_4_turn = 1;
	
	bge.logic.robot_4_turn_by = .096;
	
	bge.logic.robot_4_time_start_of_simulation = time.time( );
	
	bge.logic.robot_4_time_since_last_log = time.time( );
	...
	try:
		...
		os.remove( "./robot_logs/robot_4.log" );
		
	except OSError:
		
		pass
	
	log_file = open( "./robot_logs/robot_4.log", "a" );
...
if ( waypointString != "" ):
	...
	if ( bge.logic.robot_4_turn == 0 ): # Start moving towards the way point if the robot is done turning.

		#left_wheels = [ "robot_4_wheel_front_L", "robot_4_wheel_back_L" ];
	
		#right_wheels = [ "robot_4_wheel_front_R", "robot_4_wheel_back_R" ];
		...
		if ( ( time.time() - bge.logic.robot_4_laydown_campose_time_then ) >= 0.033989 ):
		
			bge.logic.robot_4_laydown_campose_time_then = time.time( );
		
			campose = bge.logic.getCurrentScene( ).addObject( "robot_4_campose_marker", obj );
			...
			log_file.write( str( time_stamp ) + "\t" + str( time.time( ) - bge.logic.robot_4_time_start_of_simulation ) + "\t" + str( time.time( ) - bge.logic.robot_4_time_since_last_log ) + "\t" + " CAMPOSE " + "\t" + str( obj.worldPosition.x ) + "\t" + str( obj.worldPosition.y ) + "\t" + str( orientation.to_euler( )[ 2 ] ) + "\n" ); 
		...
		if ( dist <= 1.5  ):
			...
			log_file.write( str( time_stamp ) + "\t" + str( time.time( ) - bge.logic.robot_4_time_start_of_simulation ) + "\t" + str( time.time( ) - bge.logic.robot_4_time_since_last_log ) + "\t" + " REACHED WAYPOINT " + "\t" + str( obj.worldPosition.x ) + "\t" + str( obj.worldPosition.y ) + "\t" + str( orientation.to_euler( )[ 2 ] ) + "\t" + "(" + obj[ "waypoint_coordinate" ] + ")" + "\n" ); 
	...
	# Rotating.
	...
	elif ( bge.logic.robot_4_turn != 0 ): # Start turning to face the waypoint.
		...
		if ( rotateZ < 0.0 ):
			...
			bge.logic.robot_4_turn = -1; # If angle is negative, turn the other direction.
		...
		if ( abs( rotateZ ) <= .0035 ):
			...
			bge.logic.robot_4_turn = 0; # At a certain point, stop turning and start moving forward.
		...
		obj[ "turn" ]     					 = str( bge.logic.robot_4_turn );
		obj[ "go" ]       					 = str( go );
		...
		bge.logic.robot_4_turn_by = .096 * abs( rotateZ );		
		
		if ( bge.logic.robot_4_turn_by > .096 ):
		...
		obj.applyRotation( [ 0, 0, bge.logic.robot_4_turn * ( bge.logic.robot_4_turn_by ) ], local );