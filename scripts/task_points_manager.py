...
This file draws the task points as found in
../task_points_configurations/ for [a], [b], [c],
[d], and [e] key presses.
...
# Blender calls this script each game loop so we cannot initialize
# each game loop. Thus get the "init" property of the controller
# and set it to zero that this portion of code only gets called once.
...
if (obj["init"] == 1):
  ...
  for x in range(0, len(bge.logic.task_points_configurations)):
    ...
    bge.logic.task_points_configurations[x] = [
        y for y in bge.logic.task_points_configurations[x] if y
    ]
...
if (keyboard.events[bge.events.AKEY] == activated):
  ...
  for x in range(0, len(bge.logic.task_points_configurations[0])):
    ...
    task_point = bge.logic.getCurrentScene().addObject(
        "task_point_marker", obj
    )

    task_point.worldPosition.x = float(
        bge.logic.task_points_configurations[0][x][0]
    ) / 100.0

    task_point.worldPosition.y = float(
        bge.logic.task_points_configurations[0][x][1]
    ) / 100.0

    task_point.worldPosition.z = bge.logic.getCurrentScene().objects[
        "robot_1_base"
    ].worldPosition.z + (2.0 / 100.0)
  ...
  for x in range(0, len(bge.logic.task_points_configurations[1])):
    ...
    task_point.worldPosition.x = float(
        bge.logic.task_points_configurations[1][x][0]
    ) / 100.0

    task_point.worldPosition.y = float(
        bge.logic.task_points_configurations[1][x][1]
    ) / 100.0
  ...
  for x in range(0, len(bge.logic.task_points_configurations[2])):
    ...
    task_point.worldPosition.x = float(
        bge.logic.task_points_configurations[2][x][0]
    ) / 100.0

    task_point.worldPosition.y = float(
        bge.logic.task_points_configurations[2][x][1]
    ) / 100.0
  ...
  for x in range(0, len(bge.logic.task_points_configurations[3])):
    ...
    task_point.worldPosition.x = float(
        bge.logic.task_points_configurations[3][x][0]
    ) / 100.0

    task_point.worldPosition.y = float(
        bge.logic.task_points_configurations[3][x][1]
    ) / 100.0
  ...
  for x in range(0, len(bge.logic.task_points_configurations[4])):
    ...
    task_point.worldPosition.x = float(
        bge.logic.task_points_configurations[4][x][0]
    ) / 100.0

    task_point.worldPosition.y = float(
        bge.logic.task_points_configurations[4][x][1]
    ) / 100.0
