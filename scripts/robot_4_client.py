...
class Client( asyncore.dispatcher ):
	...
	def __init__( self, host, port, message ):
		...
		try:
			...
			log_file = "../waypoint_logs/4.log";
		
		except:
			
			print( "[ROBOT_4_CLIENT.PY] no log file specified." )
			
			sys.exit( )
			
		print( "[ROBOT_4_CLIENT.PY] using " + log_file + " log file." )
		
		self.log_reader = log_reader.Log_Reader( log_file )
		
		waypoints_temp = self.log_reader.return_all_waypoints( )
		
		print( "[ROBOT_4_CLIENT.PY] sending these waypoints (" + str( len( waypoints_temp ) ) + " in total):" )
	...
	def handle_close( self ):
		...
		print '[ROBOT_4_CLIENT.PY] Closing connection.'
		
		self.close( );
		
		print '[ROBOT_4_CLIENT.PY] Exiting system.'
	...
	def handle_read( self ):
		...
		print '[ROBOT_4_CLIENT.PY] Received string:', message
		...
		if ( message == "done" ): # Server says the robot is done going to all way points.
			...
			print '[ROBOT_4_CLIENT.PY] Shutting down.'
		...
		print "[ROBOT_4_CLIENT.PY] Out Buffer contents: ", self.out_buffer;
		print "[ROBOT_4_CLIENT.PY] Out buffer size: ", len( self.out_buffer );

	# Tell asyncore what to do when the socket is ready to be written to.

	def handle_write( self ):
		
		print "[ROBOT_4_CLIENT.PY] Sending string: ", self.out_buffer;
		sent = self.send( self.out_buffer );
		print "[ROBOT_4_CLIENT.PY] Sent: ", sent;
	...
	def writable( self ):
		...
		print "[ROBOT_4_CLIENT.PY] Writable? ", ( len( self.out_buffer ) > 0 and self._writable == 1 );
	...
	def readable( self ):
		...
		print "[ROBOT_4_CLIENT.PY] Readable?", ( len( self.out_buffer ) == 0 and self._readable == 1 );
		return ( len( self.out_buffer ) == 0 and self._readable == 1 );

if __name__ == "__main__":

	client = Client( '', 5004, "hello" ) # Create the client.
